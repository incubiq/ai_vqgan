...
sys.path.insert(0, './ai/tamingt')      ## note : this dir was originally called taming-transformer
sys.path.insert(0, './ai/CLIP')
...
from tamingt.taming.models import cond_transformer, vqgan
...
#TODO : i removed this line as it could not pass with RAdam
...
def fnRun(_args): 
    ...
    vq_parser.add_argument("-width", "--width", type=int, help="Image width", default=default_image_size, dest='widthImg')
    vq_parser.add_argument("-height","--height", type=int, help="Image height ", default=default_image_size, dest='heightImg')
    ...
    vq_parser.add_argument("-filename",   "--init_image", type=str, help="Initial image", default=None, dest='init_image')
    ...
    vq_parser.add_argument("-ckpt", "--vqgan_checkpoint", type=str, help="VQGAN checkpoint", default=f'vqgan_imagenet_f16_16384.ckpt', dest='vqgan_checkpoint')
    ...
    args.size = [args.widthImg, args.heightImg]
    ...
    if args.video_style_dir:
        ...
        filename = os.path.join(args.indir, args.init_image)
        cwd = os.getcwd()
    ...
    model = load_vqgan_model("ai/checkpoints/"+args.vqgan_config, "ai/checkpoints/"+args.vqgan_checkpoint).to(device)
    ...
    if args.init_image:
        if 'http' in args.init_image:
            img = Image.open(urlopen(os.path.join(args.indir, args.init_image)))
        else:
            _localFile=os.path.abspath(os.path.join(args.indir, args.init_image))
            img = Image.open(_localFile)
    ...
    def checkin(i, losses):
        ...
        splitF=os.path.splitext(filename)
        splitF2=splitF[0]
        col=splitF[0].find(":");
        if col != -1:
            splitF2=splitF[0][0:col]
        splitF2=splitF2.replace(" ", "_")
        saveto=splitF2+"_"+str(i)+splitF[1]

        args.init_image=saveto
        args.cycle=i
        ...
        img.save(args.outdir+saveto, pnginfo=info) 	
        aFile.append(saveto)
        print("saved file: "+saveto) 	    
    ...
    def train(i):
        ...
        if (args.display_freq!=0 and i % args.display_freq == 0) or (i==args.max_iterations):
            checkin(i, lossAll)
                ...
                if i == args.max_iterations:
                    if not args.video_style_dir:
                        ...
                        if this_video_frame == (num_video_frames - 1):
                            ...
                            img = Image.open(os.path.join(args.indir, args.init_image))
                            pil_image = img.convert('RGB')
                            pil_image = pil_image.resize((sideX, sideY), Image.LANCZOS)
                            pil_tensor = TF.to_tensor(pil_image)
